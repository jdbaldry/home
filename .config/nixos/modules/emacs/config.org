...
*** Additionally, configure quelpa as a use-package handler
...
** Set PATH based upon shell PATH

#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (setenv "SHELL" "/opt/homebrew/bin/bash")
  (defun set-exec-path-from-shell-PATH ()
    (let ((path-from-shell (shell-command-to-string "$SHELL -c '. ${HOME}/.bashrc; echo ${PATH}'")))
      (setenv "PATH" path-from-shell)
      (setq exec-path (split-string path-from-shell path-separator))))
  (when window-system (set-exec-path-from-shell-PATH))
#+end_src

** Use Copilot for smarter autocomplete
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package flycheck
    ...
    (flycheck-define-checker vale-suggestion
      "A checker for prose warnings."
      :command ("~/bin/vale" "--minAlertLevel" "suggestion" source)
      :standard-input nil
      :error-patterns
    ...
    (add-to-list 'flycheck-checkers 'vale-suggestion 'append)
    (add-hook 'flycheck-error-list-mode-hook
              (lambda ()
                (setq tabulated-list-format '[("File" 20)
                                              ("Line" 5 flycheck-error-list-entry-< :right-align t)
                                              ("Col" 5 nil :right-align t)
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  ...
  (flycheck-add-next-checker 'vale-error '(error . vale-suggestion))
  (flycheck-add-next-checker 'vale-suggestion 'markdown-aspell-dynamic)
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package transient
    ...
    (transient-define-infix jdb/gh-issue:--assignee ()
      :argument "--assignee="
    ...
    (transient-define-infix jdb/gh-issue:--web ()
      :argument "--web"
    ...
    (transient-define-infix jdb/gh-issue-create:--body ()
      :argument "--body="
      :class 'transient-option
      :description "Supply a body"
      :shortarg "-b")

    (transient-define-infix jdb/gh-issue-create:--labels ()
      :argument "--labels="
      :class 'transient-option
      :description "Add comma-separated labels by name"
      :shortarg "-l")

    (defun jdb/gh--read-multiple (prompt initial-input history)
      (completing-read-multiple prompt nil nil nil nil initial-input history))

    (transient-define-infix jdb/gh-issue-create:--milestone ()
      :argument "--milestone="
      :class 'transient-option
      :description "Add the issue to a milestone by name"
      :shortarg "-m")

    (transient-define-infix jdb/gh-issue-create:--projects ()
      :argument "--projects="
      :class 'transient-option
      :description "Add the issue to comma-separated list of projects by name"
      :shortarg "-p")

    (transient-define-infix jdb/gh-issue-create:--title ()
      :argument "--title="
      :class 'transient-option
    ...
    (defun jdb/gh-issue-status:run ()
      ...
      (interactive)
      (let ((output-buffer (get-buffer-create "*GitHub Issues*"))
            (error-buffer (get-buffer-create "*GitHub errors*")))
    ...
    (transient-define-prefix jdb/gh-issue-create ()
      ...
      ["Options"
       ...
       (jdb/gh-issue:--repo)
       (jdb/gh-issue:--web)
       (jdb/gh-issue-create:--body)
       (jdb/gh-issue-create:--labels)
       (jdb/gh-issue-create:--milestone)
       (jdb/gh-issue-create:--projects)
       (jdb/gh-issue-create:--title)]
      ["Run"
       [("c" "Create" jdb/gh-issue-create:run)
        ("s" "Save" (lambda () (interactive) (message "Saved!") (transient-save)) :transient t)]])

    (defun jdb/gh--arg (arg args)
      "Return the full option of ARG as it appears in ARGS or nil."
      (let ((value (transient-arg-value arg args)))
        (cond ((stringp value) (concat arg value))
              (value arg))))

    (defun jdb/gh--repeat-arg (arg singular args)
      "Return the comma-separated values for ARG as it appears in ARGS joined by the option SINGULAR, or nil."
      (let ((values (transient-arg-value arg args)))
        (if values
            (string-join (mapcar (lambda (value) (concat singular value)) (string-split values ",")) " ")
          nil)))

    (transient-define-suffix jdb/gh-issue-create:run ()
      "Create an issue on GitHub."
      (interactive)
      (let* ((output-buffer (get-buffer-create "*GitHub TODO*"))
             (error-buffer (get-buffer-create "*GitHub errors*"))
             (args (transient-args 'jdb/gh-issue-create))
             (assignee (jdb/gh--arg "--assignee=" args))
             (repo (jdb/gh--arg "--repo=" args))
             (web (jdb/gh--arg "--web=" args))
             (body (jdb/gh--arg "--body=" args))
             (labels (jdb/gh--repeat-arg "--labels=" "--label=" args))
             (milestone (jdb/gh--arg "--milestone=" args))
             (projects (jdb/gh--repeat-arg "--projects=" "--project=" args))
             (title (jdb/gh--arg "--title=" args)))
        (jdb/gh--shell-command (jdb/list-to-shell-string
                                `("gh" "issue" ,repo
                                  "create" ,assignee ,body ,labels ,milestone ,projects ,title))
                               output-buffer
                               error-buffer)
        (switch-to-buffer output-buffer)))
    ...
    (defun jdb/gh-pr-status ()
      ...
      (interactive)
      (let ((output-buffer (get-buffer-create "*GitHub PRs*"))
            (error-buffer (get-buffer-create "*GitHub errors*")))
        (jdb/gh--shell-command (jdb/list-to-shell-string
    ...
    (transient-define-infix jdb/gh-pr:number ()
      ...
      :init-value (lambda (obj)
                    (oset obj value (let ((guess (string-trim (or (caddr (split-string (thing-at-point 'line) "|")) ""))))
    ...
    (defun jdb/gh-pr-view:run ()
      "View the GitHub PR near point in a new buffer."
      (interactive)
      (let ((output-buffer (get-buffer-create "*GitHub PR*"))
            (error-buffer (get-buffer-create "*GitHub errors*"))
  ...
  (use-package transient
    ...
    (transient-define-prefix jdb/rm ()
      ...
      (ivy-read "Remove file: "
                ...
                :caller 'jdb/rm))
    (transient-define-prefix jdb/ci-ci ()
      (interactive)
      (magit-run-git "ci-ci"))
       ...
       (cl-function
        (lambda (&key response &allow-other-keys)
          (if (and
               ...
               (equal (alist-get 'ok (request-response-data response)) :json-true)
...
** Conveniently share links to central repositories from source files
...
** Use web-mode for HTML templates

#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package web-mode
    :config
    (progn (setq-local web-mode-indent-style 2)
           (setq-local require-final-newline nil)
           (setq-local web-mode-engine "go")
           (setq web-mode-markup-indent-offset 2)
           (setq-local web-mode-code-indent-offset 0)
           (setq-local mode-require-final-newline nil)
           (setq-local indent-tabs-mode nil)
           (setq-local tab-width 2)))
#+end_src

** Workaround MacOS file 1024 file limit

#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (defun jdb/file-notify-rm-all-watches ()
...
** Use deadgrep for fast searching

#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package deadgrep :ensure t)
#+end_src

** Insert columns for Vale flycheck output

#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (defun jdb/flycheck-vale-columns ()
    "Insert columns for Vale flycheck output."
    (interactive)
    (insert "| FILE | LINE | COL | LEVEL | RULE | MESSAGE |")
    (insert "| ---- | ---- | --- | ----- | ---- | ------- |"))
#+end_src
